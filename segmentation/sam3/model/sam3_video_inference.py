...
class Sam3VideoInference(Sam3VideoBase):
    ...
    def _postprocess_output(
        ...
        if len(curr_obj_ids) == 0:
            ...
            # 2. 判斷設備類型來決定是否使用 pin_memory()
            keep_idx = torch.nonzero(keep, as_tuple=True)[0]

            if target_device.type == 'cuda':
                keep_idx_gpu = keep_idx.pin_memory().to(device=target_device, non_blocking=True)
            else:
                keep_idx_gpu = keep_idx.to(device=target_device, non_blocking=True)
            # keep_idx = torch.nonzero(keep, as_tuple=True)[0]
