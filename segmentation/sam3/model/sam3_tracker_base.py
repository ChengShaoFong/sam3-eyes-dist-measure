...
class Sam3TrackerBase(torch.nn.Module):
    ...
    def _get_tpos_enc(self, rel_pos_list, device, max_abs_pos=None, dummy=False):
        ...
        pos_enc_tensor = torch.tensor(rel_pos_list)

        if device.type == 'cuda':
            pos_enc = pos_enc_tensor.pin_memory().to(device=device, non_blocking=True) / t_diff_max
        else:
            pos_enc = pos_enc_tensor.to(device=device, non_blocking=True) / t_diff_max
        # pos_enc = (
